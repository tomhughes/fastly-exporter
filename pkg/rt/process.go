...
func process(rt RealtimeResponse, serviceID, serviceName, serviceVersion string, m *prom.Metrics) {
	for _, d := range rt.Data {
		for datacenter, stats := range d.Datacenter {
			...
			m.MissHistogram.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.MissHistogram))
			...
			m.Status206.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.Status206))
			m.Status429.WithLabelValues(serviceID, serviceName, datacenter).Add(float64(stats.Status429))
